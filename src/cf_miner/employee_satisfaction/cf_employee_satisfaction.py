...
INPUT_CSV = PROJECT_ROOT / "out" / "cf_miner" / "prepared_cf_employee_satisfaction.csv"
OUTPUT_DIR = PROJECT_ROOT / "out" / "cf_miner" / "employee_satisfaction_top10"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

TARGET = "employee_satisfaction_ord"
TOP_N = 10
...
def print_czech_interpretation() -> None:
    cz_text = (
        ...
        "ve spokojenosti zaměstnanců. "
        "Nejsilnější nalezená pravidla ukazují, že vyšší spokojenost zaměstnanců je nejvíce spojena s intenzivnějším využíváním AI "
        "v kombinaci s lepší přípravou pracovní síly na tyto změny. "
        "Obzvláště silné vzory se objevují u firem s vysokou mírou adopce AI a vyšším počtem aktivních AI projektů, "
        "ale také u firem, které více investují do školení zaměstnanců a vykazují vyšší počet rekvalifikovaných pracovníků. "
        "Dalším podpůrným faktorem je vyšší úroveň řízení AI rizik a v menší míře také přítomnost AI etické komise. "
        "Výsledky tedy naznačují, že při srovnatelné úrovni automatizace není vyšší spokojenost zaměstnanců spojena pouze "
        "se samotnou přítomností AI, ale především s tím, že organizace AI zavádí aktivně, systematicky a současně dokáže zaměstnance "
        "na tyto změny dobře připravit."
    )
    ...
    clm = cleverminer(
        ...
        cond={
            "attributes": [
                ...
                {"name": "ai_training_hours_ord", "type": "seq", "minlen": 1, "maxlen": 1},
                {"name": "reskilled_employees_ord", "type": "seq", "minlen": 1, "maxlen": 1},
